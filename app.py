...
def find_matches_for_suggestion(df, suggestion):
    """
    Given an AI-corrected suggestion string, try multiple strategies
    to find matching records in the database, going from strict to loose.

    Strategy 1 — Full suggestion match:   "betadine solution" -> search "betadine solution"
    Strategy 2 — First word only:         "betadine solution" -> search "betadine"
    Strategy 3 — Each word independently: try each word that is >= 4 chars long

    Returns the matches dataframe (may be empty) and the token that matched.
    """
    suggestion_clean = suggestion.strip().lower()

    # Strategy 1: full string match
    matches = df[df['name_clean'].str.contains(suggestion_clean, na=False, regex=False)]
    ...
    # Strategy 3: any meaningful word in the suggestion
    for word in words[1:]:
        if len(word) >= 4:
            matches = df[df['name_clean'].str.contains(word, na=False, regex=False)]
            if not matches.empty:
                return matches, word

    return pd.DataFrame(), suggestion_clean
...
if search_query:
    ...
    matches = df[df['name_clean'].str.contains(query_clean, na=False, regex=False)]
    ...
    if not matches.empty:
        ...
        # No direct match — call the AI spelling correction layer
        ...
        if suggestion:
            ...
            if not ai_matches.empty:
                ...
                st.error(
                    f"We found **'{suggestion}'** as a possible correction, "
                    f"but it isn't in our database. Please try a different name."
                )
        else:
            st.error(f"**'{search_query}'** not found. Please verify the spelling.")
...
if selected_medicine:
    ...
    with st.expander("📊 Customize Your Savings Calculation", expanded=True):
        c1, c2 = st.columns(2)
        with c1:
            u_p = st.number_input("Units per purchase:", min_value=1, value=10)
        with c2:
            p_m = st.number_input("Purchases per month:", min_value=1, value=1)
    ...
    if results:
        ...
        for _, row in res_df.iterrows():
            with st.container(border=True):
                ...
                with col2:
                    st.metric("Price/Unit", f"₹{row['price']:.2f}")
                with col3:
                    if row['yearly'] > 0:
                        st.markdown(f"### Savings: ₹{row['yearly']:.2f}/yr")
    else:
